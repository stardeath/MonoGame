...
namespace Microsoft.Xna.Framework.Graphics
{
	...
	public class SpriteBatch : GraphicsResource
	{
		...
		/// <param name="sourceRectangle">An optional region on the texture which will be rendered. If null - draws full texture.</param>
		/// <param name="color">A color mask.</param>
		/// <param name="origin">Coordinates to center the texture on.</param>
            ...
            float layerDepth)
        {
            ...
            Vector2 position = destinationRectangle.Location.ToVector2() + destinationRectangle.Size.ToVector2() / 2.0f;
            ...
            var l = position.X - origin.X;
            var t = position.Y - origin.Y;
            var r = l + w;
            var b = t + h;

            if (l < destinationRectangle.Left)
            {
                float factor = MathHelper.InverseLerp(l, r, destinationRectangle.Left);
                l = destinationRectangle.Left;
                _texCoordTL.X = MathHelper.Lerp(_texCoordTL.X, _texCoordBR.X, factor);
            }

            if (t < destinationRectangle.Top)
            {
                float factor = MathHelper.InverseLerp(t, b, destinationRectangle.Top);
                t = destinationRectangle.Top;
                _texCoordTL.Y = MathHelper.Lerp(_texCoordTL.Y, _texCoordBR.Y, factor);
            }

            if (r > destinationRectangle.Right)
            {
                float factor = MathHelper.InverseLerp(l, r, destinationRectangle.Right);
                r = destinationRectangle.Right;
                _texCoordBR.X = MathHelper.Lerp(_texCoordTL.X, _texCoordBR.X, factor);
            }

            if (b > destinationRectangle.Bottom)
            {
                float factor = MathHelper.InverseLerp(t, b, destinationRectangle.Bottom);
                b = destinationRectangle.Bottom;
                _texCoordBR.Y = MathHelper.Lerp(_texCoordTL.Y, _texCoordBR.Y, factor);
            }

            item.Set(l, t, r - l, b - t, color, _texCoordTL, _texCoordBR, layerDepth);
